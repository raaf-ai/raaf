  ...
  module Tracing
    ...
    class DashboardController < ApplicationController
      ...
      def index
        ...
        # Top workflows
        @top_workflows = TraceRecord.top_workflows(limit: 10, timeframe: @time_range)

        # Recent activity
        @recent_traces = TraceRecord.recent.limit(10).includes(:spans)
        @recent_errors = SpanRecord.errors.recent.limit(10).includes(:trace)
      ...
      def performance
        ...
        @performance_by_kind = %w[agent llm tool handoff].to_h do |kind|
          metrics = SpanRecord.performance_metrics(kind: kind, timeframe: @time_range)
          [kind, metrics]
        end

        # Slowest operations
        @slowest_spans = SpanRecord.slow(1000).within_timeframe(@time_range.begin, @time_range.end)
                                   .includes(:trace)
      ...
      def costs
        ...
        @cost_analysis = SpanRecord.cost_analysis(timeframe: @time_range)

        # Calculate actual costs using CostManager
        traces = TraceRecord.within_timeframe(@time_range.begin, @time_range.end)
        total_cost = 0.0
      ...
      def errors
        ...
        # Error analysis
        @error_analysis = SpanRecord.error_analysis(timeframe: @time_range)

        # Error trends over time
        @error_trends = calculate_error_trends(@time_range)

        # Recent errors with details
        @recent_errors = SpanRecord.errors.within_timeframe(@time_range.begin, @time_range.end)
                                   .includes(:trace)
                                   .recent
                                   .limit(50)
      ...
      def calculate_overview_stats(time_range)
        traces = TraceRecord.within_timeframe(time_range.begin, time_range.end)
        spans = SpanRecord.within_timeframe(time_range.begin, time_range.end)
      ...
      def calculate_performance_trends(time_range)
        ...
        while current_time < time_range.end
          ...
          traces_in_bucket = TraceRecord.within_timeframe(current_time, bucket_end)
      ...
      def calculate_performance_over_time(time_range)
        ...
        while current_time < time_range.end
          ...
          spans_in_bucket = SpanRecord.within_timeframe(current_time, bucket_end)
      ...
      def calculate_cost_by_model(time_range)
        llm_spans = SpanRecord.by_kind("llm").within_timeframe(time_range.begin, time_range.end)
      ...
      def calculate_usage_over_time(time_range)
        ...
        while current_time < time_range.end
          ...
          llm_spans = SpanRecord.by_kind("llm").within_timeframe(current_time, bucket_end)
