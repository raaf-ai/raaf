...
module OpenAIAgents
  module Tracing
    ...
    #   OpenAIAgents::Tracing::Span.slow(threshold: 1000) # > 1 second
    #
    # @example Find error spans
    #   OpenAIAgents::Tracing::Span.errors.recent
    #
    # @example Find spans by operation type
    #   OpenAIAgents::Tracing::Span.by_kind('llm')
    #
    # @example Get span performance metrics
    #   OpenAIAgents::Tracing::Span.performance_metrics('tool')
    class SpanRecord < ActiveRecord::Base
      self.table_name = "openai_agents_tracing_spans"

      # Associations
      belongs_to :trace, primary_key: :trace_id, foreign_key: :trace_id,
                         class_name: "OpenAIAgents::Tracing::TraceRecord", optional: true
      belongs_to :parent_span, class_name: "OpenAIAgents::Tracing::SpanRecord",
                               primary_key: :span_id, foreign_key: :parent_id, optional: true
      has_many :children, class_name: "OpenAIAgents::Tracing::SpanRecord",
        ...
        def error_analysis(timeframe: nil)
          ...
          if error_spans.any?
            ...
            workflow_data = error_spans.joins(:trace)
                                       .pluck("openai_agents_tracing_traces.workflow_name", "openai_agents_tracing_spans.id") # rubocop:disable Layout/LineLength
      ...
      def update_trace_status
        ...
        Rails.logger.warn "[OpenAI Agents Tracing] Failed to update trace status: #{e.message}"
      end
    end
  end
end
