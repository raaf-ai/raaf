...
RSpec.describe RAAF::Tracing::SpanCollectors::BaseCollector do
  describe "DSL methods" do
    describe ".span" do
      it "registers simple attributes" do
        ...
        expect(test_class.instance_variable_get(:@span_attrs)).to eq(%i[name model])
      end
  ...
  describe "#collect_attributes" do
    ...
    it "executes lambda-based custom attributes" do
      ...
      allow(component).to receive(:tools).and_return(%w[tool1 tool2])
  ...
  describe "#collect_result" do
    ...
    it "includes custom result attributes from DSL" do
      test_class = Class.new(described_class) do
        result execution_result: ->(result, _component) { result.to_s[0..10] }
        result status: ->(_result, _component) { "success" }
      end
