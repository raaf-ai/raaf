...
RSpec.describe RAAF::Tracing::Traceable do
  ...
  describe "class methods" do
    ...
    describe ".trace_component_type" do
      ...
      context "when declared on a superclass" do
        let(:base_class) do
          Class.new do
            include RAAF::Tracing::Traceable
        ...
        it "inherits the declaration instead of inferring from the class name" do
          subclass = Class.new(base_class)
          allow(subclass).to receive(:name).and_return("Ai::SearchProviders::Perplexity")

          expect(subclass.trace_component_type).to eq(:custom)
        end

        it "inherits the declaration through several levels" do
          grandchild = Class.new(Class.new(base_class))

          expect(grandchild.trace_component_type).to eq(:custom)
        end

        it "lets a subclass override the inherited declaration" do
          subclass = Class.new(base_class) { trace_as :tool }

          expect(subclass.trace_component_type).to eq(:tool)
        end

        it "still infers when no ancestor declared a type" do
          plain_base = Class.new { include RAAF::Tracing::Traceable }
          subclass = Class.new(plain_base)
          allow(subclass).to receive(:name).and_return("MyGenericSubclass")

          expect(subclass.trace_component_type).to eq(:component)
        end
      end
    end
  end
  ...
  describe "smart span lifecycle management" do
    ...
    describe "span duplicate prevention" do
      ...
      it "cleans up old sent span tracking to prevent memory leaks" do
        ...
        1100.times do |i|
          agent.with_tracing(:"method_#{i}") do
            # Block execution
          end
        end
  ...
  describe "get_tracer_for_span_sending (TracingRegistry integration)" do
    ...
    before do
      ...
      if defined?(RAAF::Tracing::TraceProvider)
        allow(RAAF::Tracing::TraceProvider).to receive(:instance).and_raise(StandardError,
                                                                            "TraceProvider not available")
      end
    end
