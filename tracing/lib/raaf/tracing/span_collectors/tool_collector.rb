  ...
  module Tracing
    ...
    module SpanCollectors
      ...
      class ToolCollector < BaseCollector
        ...
        span agent_context: lambda { |comp|
          comp.detect_agent_context&.class&.name if comp.respond_to?(:detect_agent_context)
        }
        ...
        span "retry.total_backoff_ms": lambda { |comp|
        ...
        result status: lambda { |result, _comp|
        ...
        result "duration.ms": lambda { |_result, comp|
          comp.execution_duration_ms.to_s if comp.respond_to?(:execution_duration_ms)
        }

        # Result size in bytes (for large result handling)
        # Stored as: result.size_bytes
        # @return [String] Size in bytes or empty
        result "size.bytes": lambda { |result, _comp|
        ...
        result "error.type": lambda { |result, _comp|
        ...
        result "error.message": lambda { |result, _comp|
          if result.is_a?(Exception)
            result.message
          elsif result.respond_to?(:error) && result.error
        ...
        result execution_result: lambda { |result, _comp|
          if result.is_a?(Exception)
        ...
        result tool_result: lambda { |result, _comp|
          if result.is_a?(Exception)
