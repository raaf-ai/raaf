...
module RAAF
  ...
  module Tracing
    ...
    ##
    # Visualization System for RAAF Tracing
    #
    ...
    #   visualizer = RAAF::Tracing::Visualization::TraceVisualizer.new(spans)
    #   puts visualizer.render_ascii
    #   puts visualizer.render_timeline
    #
    # @example HTML report generation
    #   html_report = RAAF::Tracing::Visualization::HTMLVisualizer.generate(spans)
    #   File.write("trace_report.html", html_report)
    #
    # @example Workflow visualization
    #   workflow_viz = RAAF::Tracing::Visualization::WorkflowVisualizer.new(agents)
    ...
    #   chart = RAAF::Tracing::Visualization::MetricsChart.generate_performance_chart(performance_data)
    #   puts chart
    #
    # @author RAAF (Ruby AI Agents Factory) Team
    # @since 0.1.0
    # @see RAAF::Tracing::SpanTracer For trace data collection
    module Visualization
      ...
      class TraceVisualizer
        ...
        def generate_mermaid
          ...
          @spans.each do |span|
            ...
            duration = if span.is_a?(Hash)
                         span[:duration_ms]
                       else
                         (span.respond_to?(:duration_ms) ? span.duration_ms : "unknown")
                       end
        ...
        def render_spans_ascii(tree_nodes, output, depth)
          tree_nodes.each_with_index do |node, index|
            ...
            attributes = if span.is_a?(Hash)
                           span[:attributes] || {}
                         else
                           (span.respond_to?(:attributes) ? span.attributes : {})
                         end
            if attributes.any?
      ...
      class HTMLVisualizer
        ...
        def self.prepare_spans_data(spans)
          spans.map do |span|
            {
              span_id: span.is_a?(Hash) ? span[:span_id] : span.span_id,
              trace_id: if span.is_a?(Hash)
                          span[:trace_id]
                        else
                          (span.respond_to?(:trace_id) ? span.trace_id : nil)
                        end,
              parent_id: span.is_a?(Hash) ? span[:parent_id] : span.parent_id,
              ...
              duration: if span.is_a?(Hash)
                          ...
                          (span.respond_to?(:duration) && span.duration ? (span.duration * 1000).round(2) : 0)
                        end,
              status: if span.is_a?(Hash)
                        span[:status]
                      else
                        (span.respond_to?(:status) ? span.status : nil)
                      end,
              attributes: if span.is_a?(Hash)
                            span[:attributes] || {}
                          else
              ...
              events: if span.is_a?(Hash)
                        span[:events] || []
                      else
                        (span.respond_to?(:events) ? span.events : [])
                      end
            }
          end
        end
      ...
      # @author RAAF (Ruby AI Agents Factory) Team
      # @since 0.1.0
      # @see RAAF::Agent For agent configuration
