...
class CodeValidator
  ...
  def extract_code_blocks(file_pattern = "*.md")
    ...
    Dir.glob(@guides_dir.join(file_pattern)).each do |file_path|
      ...
      code_blocks.concat(extract_ruby_blocks(file_content, relative_path)).each do |block|
        block.instance_variable_set(:@validator, self)
      end
    end
  ...
  def unmark_all_examples!
    ...
    Dir.glob(@guides_dir.join("*.md")).each do |file_path|
      ...
      content.gsub!(
        /WARNING: \*\*VALIDATION FAILED\*\* - This example needs work and contributions are welcome! Please see \[Contributing to RAAF\]\(contributing_to_raaf\.md\) for guidance\.\n\n\*\*Error\*\*: .+\n\n/, ""
      )
      content.gsub!(
        /WARNING: \*\*VALIDATION FAILED\*\* - This example needs work and contributions are welcome! Please see \[Contributing to RAAF\]\(contributing_to_raaf\.md\) for guidance\. Error: .+\n\n/, ""
      )
      content.gsub!(
        /WARNING: \*\*EXAMPLE VALIDATION FAILED\*\* - This example needs work and contributions are welcome! Please see \[Contributing to RAAF\]\(contributing_to_raaf\.md\) for guidance\. Error: .+\n\n/, ""
      )
      content.gsub!(
  ...
  def mark_file_failures(file_path, failed_results)
    ...
    failed_results.each do |result|
      ...
      next unless ruby_block_start > 0 && ruby_block_start <= lines.length
