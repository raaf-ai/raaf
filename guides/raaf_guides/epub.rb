...
require "raaf_guides/epub_packer"
...
module Epub # :nodoc:
    ...
    Dir.chdir output_dir do
      ...
      File.write("META-INF/container.xml", <<~CONTENT)
        ...
        <container xmlns="urn:oasis:names:tc:opendocument:xmlns:container" version="1.0">
          <rootfiles>
              <rootfile full-path="OEBPS/raaf_guides.opf" media-type="application/oebps-package+xml"/>
          </rootfiles>
        </container>
      CONTENT
    end
  ...
  def fix_file_names(output_dir)
    ...
    Dir.chdir book_dir do
      ...
      opf = File.read("raaf_guides.opf")

      doc = open_toc_doc(toc)
      doc.each do |c|
        name = c[:src]

        next unless is_name_invalid(name)
      ...
      File.write("raaf_guides.opf", opf)
    end
  end
