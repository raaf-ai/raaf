...
require "raaf_guides/markdown"
require "raaf_guides/helpers"
require "raaf_guides/epub"

module RailsGuides
  class Generator
    GUIDES_RE = /\.(?:erb|md)\z/

    def initialize(edge:, version:, all:, only:, epub:, language:, lint:, direction: nil)
      @edge         = edge
      @version      = version
      @all          = all
      @only         = only
      ...
      register_special_mime_types if @epub

      initialize_dirs
      create_output_dir_if_needed unless dry_run?
    ...
    def epub_filename
      epub_filename = "ruby_on_raaf_guides_#{@version || @edge[0, 7]}"
    ...
    def process_scss
      # Use array form of system to properly handle paths with spaces
      system(
    ...
    def generate_guide(guide, output_file)
      ...
      if guide =~ /\.(\w+)\.erb$/
        return if %w[_license _welcome layout].include?(::Regexp.last_match.pre_match)

        # Generate the special pages like the home.
        # Passing a template handler in the template name is deprecated. So pass the file name without the extension.
        result = view.render(layout: layout, formats: [::Regexp.last_match(1).to_sym],
                             template: ::Regexp.last_match.pre_match)
      else
        ...
        @warnings << "[WARN] BROKEN LINK(s): #{guide}: #{broken.join(", ")}" if broken.any?
      end

      File.write(output_path, result) unless dry_run?
    end
    ...
    def check_fragment_identifiers(html, anchors)
      ...
      html.scan(/<a\s+href="#([^"]+)/).flatten.each do |fragment_identifier|
        ...
        next if anchors.member?(CGI.unescape(fragment_identifier))
