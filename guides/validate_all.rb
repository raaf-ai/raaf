...
class GuideValidator
  ...
  def validate_guide_structure
    ...
    required_sections = {
      ...
      "core_guide.md" => %w[Introduction Agents Runners Tools],
      "api_reference.md" => %w[Classes Methods]
    }
    ...
    required_sections.each do |file, sections|
      ...
      sections.each do |section|
        missing_sections << "#{file}: Missing '#{section}' section" unless content.include?(section)
      end
    end
  ...
  def validate_cross_references
    ...
    Dir.glob("source/*.md").each do |file|
      ...
      internal_links.each do |_text, link|
        # Convert to source file path
        source_file = link.gsub(".html", ".md")
        source_path = "source/#{source_file}"

        broken_links << "#{file}: Broken link to #{link}" unless File.exist?(source_path)
      end
    end
