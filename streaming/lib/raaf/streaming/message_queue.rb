...
module RAAF
  module Streaming
    ...
    class MessageQueue
      ...
      #
      def initialize(redis_url: "redis://localhost:6379", max_size: 10000, batch_size: 100, queue_name: "raaf_messages")
        @redis_url = redis_url
        @max_size = max_size
      ...
      def enqueue(message, priority: :normal, ttl: nil)
        ...
        # Check queue size
        if @redis.llen(queue_key) >= @max_size
          raise StandardError, "Queue size limit exceeded"
        end
