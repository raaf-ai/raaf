...
module RAAF
  module Streaming
    ...
    class RealTimeChat
      ...
      def handle_stop_stream(client, session_id, message)
      ...
      def handle_leave_room(client, message)
        ...
        if leave_room(client.id, room_id)
          client.send_message({
            type: "room_left",
            room_id: room_id,
            timestamp: Time.current.iso8601
      ...
      def add_to_history(session_id, message)
        @mutex.synchronize do
          ...
          if @message_history[session_id].size > max_history
            @message_history[session_id].shift(@message_history[session_id].size - max_history)
          end
        end
      end
    end
