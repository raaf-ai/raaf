  ...
  module Guardrails
    ...
    class SecurityGuardrail < Base
      ...
      def perform_check(content, _context)
        ...
        violations.concat(detect_prompt_injection(content)) if @detection_types.include?(:prompt_injection)

        # Check for encoded malicious content
        violations.concat(detect_encoded_content(content)) if @detection_types.include?(:encoded_content)

        # Check for malicious URLs
        violations.concat(detect_malicious_urls(content)) if @detection_types.include?(:malicious_urls)
      ...
      def default_detection_types
        %i[prompt_injection encoded_content malicious_urls]
      end
      ...
      def detect_encoded_content(content)
        ...
        words.each do |word|
          ...
          ENCODED_PATTERNS.each do |encoding_type, pattern|
            next unless word.match?(pattern)
