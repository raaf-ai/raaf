...
module RAAF
  ...
  module Guardrails
    ...
    # Scans content for Personally Identifiable Information (PII) by pattern.
    #
    # This is the low-level scanner the compliance guardrails build on: it walks a
    # set of regexes over a string and redacts or reports what it matches. For
    # confidence scoring, contextual analysis and sector-specific identifiers, use
    # {PIIDetector} instead.
    class PIIScanner < Base
      ...
      DEFAULT_PATTERNS = {
        ...
        date_of_birth: {
          pattern: %r{\b(?:DOB|Date of Birth|Born on):?\s*\d{1,2}[-/]\d{1,2}[-/]\d{2,4}\b}i,
      ...
      def perform_check(content, _context)
        ...
        @patterns.each do |type, config|
          ...
          matches.each do |match|
            ...
            modified_content.gsub!(match_text, "#{@redaction_token}:#{type}") if @action == :redact
          end
        end
