...
module RAAF
  ...
  module Guardrails
    ...
    # @see RAAF::Guardrails::PIIScanner For plain pattern scanning inside a compliance guardrail
    class PIIDetector
      ...
      attr_reader :name, :sensitivity_level, :custom_patterns, :redaction_enabled, :detection_stats
      ...
      def detect_pii(text, source = "unknown")
        ...
        PII_PATTERNS.merge(@custom_patterns).each do |type, config|
          ...
          each_match(text, config[:pattern]) do |match_str, position|
      ...
      # Yield every match of +pattern+ in +text+ as [matched string, offset].
      #
      # String#scan hands back the capture groups when a pattern has any, which
      # loses both the text that was actually matched and where it sat, so walk
      # the string with MatchData instead.
      def each_match(text, pattern)
        offset = 0

        while (match = pattern.match(text, offset))
          yield match[0], match.begin(0)
          offset = match.end(0) > match.begin(0) ? match.end(0) : match.begin(0) + 1
        end
      end
      ...
      def deduplicate_detections(detections)
        # Remove overlapping detections, keeping the most specific one. A sector
        # or caller-supplied pattern wins over the general set even when it
        # claims less confidence: a nine-digit number in a financial document is
        # reported as a routing number rather than as the SSN it also looks like.
        detections.sort_by { |d| [@custom_patterns.key?(d[:type]) ? 0 : 1, -d[:confidence], d[:position]] }
    ...
    class FinancialPIIDetector < PIIDetector
      ...
      FINANCIAL_PATTERNS = {
        ...
        tax_id: {
          # An EIN is written with the separator; without it the digits are
          # indistinguishable from an SSN or a routing number.
          pattern: /\b\d{2}-\d{7}\b/,
          name: "Tax ID/EIN",
          confidence: 0.8,
          validator: ->(match) { match.gsub(/\D/, "").length == 9 }
