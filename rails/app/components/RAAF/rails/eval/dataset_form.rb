      ...
      class DatasetForm < RAAF::Rails::Tracing::BaseComponent
        ...
        def view_template
          div(class: "p-6 max-w-2xl") do
            ...
            form_with(model: @dataset, url: @dataset.new_record? ? eval_datasets_path : eval_dataset_path(@dataset),
                      method: @dataset.new_record? ? :post : :patch) do |f|
        ...
        def render_form_fields(f)
          div(class: "space-y-4") do
            ...
            div(class: "flex gap-3 pt-4") do
              f.submit(@dataset.new_record? ? "Create Dataset" : "Update Dataset",
