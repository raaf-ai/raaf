      ...
      class PromptForm < RAAF::Rails::Tracing::BaseComponent
        ...
        def view_template
          div(class: "p-6 max-w-2xl") do
            ...
            form_with(model: @prompt, url: @prompt.new_record? ? eval_prompts_path : eval_prompt_path(@prompt),
                      method: @prompt.new_record? ? :post : :patch) do |f|
        ...
        def render_form_fields(f)
          div(class: "space-y-4") do
            div do
              ...
              f.text_field :name,
                           class: "mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm", required: true, placeholder: "e.g. customer_support_prompt"
            end
            div do
            ...
            if @prompt.new_record?
              div(class: "border-t border-gray-200 pt-4") do
                ...
                div do
                  ...
                  textarea(name: "prompt[initial_content]", rows: 6,
                           class: "mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm font-mono", placeholder: "You are a helpful assistant...")
                end
                div do
                  label(class: "block text-sm font-medium text-gray-700") { "Target Model" }
                  input(type: "text", name: "prompt[initial_model]",
                        class: "mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm", placeholder: "e.g. gpt-4o")
                end
              end
            end

            div(class: "flex gap-3 pt-4") do
              f.submit(@prompt.new_record? ? "Create Prompt" : "Update Prompt",
