...
The engine serves none. The `/api/v1` namespace it used to mount under the
console answered a fixed payload to every call -- an empty agent list, an agent
named after whatever id you asked for, a chat that replied with the same
sentence -- over records the engine does not have. It was removed rather than
implemented, because there is no agent store behind it to implement it against.
A host application that wants an agent API builds one over its own models.
