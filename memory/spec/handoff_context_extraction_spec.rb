...
RSpec.describe "OpenAIAgents Handoff Context Extraction" do
  ...
  let(:context_wrapper) { RAAF::RunContextWrapper.new(RAAF::RunContext.new) }

  before do
    source_agent.add_handoff(target_agent)

    # Mock the provider
    allow(RAAF::Models::ResponsesProvider).to receive(:new).and_return(mock_provider)

    # Add context_storage_keys method to source agent for testing
    def source_agent.context_storage_keys
      %w[user_info conversation_summary extracted_data]
    end
  end
  ...
  describe "Context Extraction During Handoff" do
    context "when agent response contains JSON with context data" do
      ...
      it "extracts context data from JSON response before handoff" do
        ...
        last_assistant_item = generated_items.reverse.find do |item|
          item.is_a?(RAAF::Items::MessageOutputItem) &&
      ...
      before do
        ...
        def target_agent.get_instructions(context_wrapper)
          ...
          if context_wrapper
            ...
            base_instructions += "\n\nConversation Summary: #{conversation_summary}" unless conversation_summary.empty?
          end
