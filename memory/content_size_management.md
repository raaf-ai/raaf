# Content Size Management and Automatic Summarization for RAAF (Ruby AI Agents Factory)

This document describes strategies for managing content size when dealing with large numbers of tool calls and conversations in RAAF (Ruby AI Agents Factory), along with a comprehensive automatic summarization system.
...
When dealing with large numbers of tool calls in RAAF (Ruby AI Agents Factory), several content size issues can arise:
...
class Runner
  def initialize(agent:, provider: nil, tracer: nil, disabled_tracing: false, 
    ...
    @tracer = tracer || (@disabled_tracing ? nil : RAAF.tracer)
...
```ruby
summarization_config = RAAF::Summarization::Config.new
...
```ruby
triggers = RAAF::Summarization::AutomaticTriggers.new
...
```ruby
manual_trigger = RAAF::Summarization::ManualTrigger.new
...
These strategies help manage content size in RAAF (Ruby AI Agents Factory):
