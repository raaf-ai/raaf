...
# This example demonstrates lifecycle hooks in RAAF (Ruby AI Agents Factory).
...
class LoggingRunHooks < RAAF::RunHooks
  ...
  def on_tool_start(_context, agent, tool, arguments)
  ...
  def on_error(_context, agent, error)
...
class MetricsAgentHooks < RAAF::AgentHooks
  ...
  def on_end(context, agent, _output)
...
runner = RAAF::Runner.new(agent: general_agent)
...
class ContextTrackingHooks < RAAF::RunHooks
...
class ErrorHandlingHooks < RAAF::RunHooks
...
runner = RAAF::Runner.new(agent: error_agent)
