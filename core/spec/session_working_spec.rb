...
RSpec.describe RAAF::Session do
  ...
  describe "#summary" do
    ...
    it "returns comprehensive session summary" do
      ...
      expect(summary[:roles]).to eq(["user", "assistant"])
    end
...
RSpec.describe RAAF::InMemorySessionStore do
  ...
  describe "#list_sessions" do
    ...
    it "returns all session IDs" do
      ...
      expect(session_ids.sort).to eq(["session-1", "session-2"])
    end
  end
  ...
  describe "#stats" do
    it "returns comprehensive statistics" do
      ...
      expect(stats[:session_ids].sort).to eq(["stats-1", "stats-2"])
...
RSpec.describe RAAF::FileSessionStore do
  ...
  describe "#list_sessions" do
    ...
    it "returns all session IDs from JSON files" do
      ...
      session_ids = store.list_sessions
      expect(session_ids.sort).to eq(["file-session-1", "file-session-2"])
    end
  ...
  describe "#stats" do
    it "returns comprehensive file store statistics" do
      ...
      expect(stats[:total_sessions]).to eq(2)
      expect(stats[:session_ids].sort).to eq(["stats-file-1", "stats-file-2"])
      expect(stats[:total_messages]).to eq(3)
