...
RSpec.describe RAAF::Logging do
  ...
  describe ".info" do
    before do
      # Configure log level to allow info messages
      described_class.configure { |c| c.log_level = :info }
    end
  ...
  describe ".warn" do
    before do
      # Configure log level to allow warn messages
  ...
  describe ".error" do
    before do
      # Configure log level to allow error messages
      described_class.configure { |c| c.log_level = :error }
    end
  ...
  describe "specialized logging methods" do
    before do
      # Configure log level to allow info messages for specialized methods
      described_class.configure { |c| c.log_level = :info }
    end
  ...
  describe ".benchmark" do
    before do
      # Configure log level to allow info messages for benchmark
      described_class.configure { |c| c.log_level = :info }
    end
...
RSpec.describe RAAF::Logging::Configuration do
  ...
  describe "#initialize" do
    ...
    it "loads configuration from environment variables" do
      ...
      expect(new_config.debug_categories).to eq(%i[api tracing])
    end
  end
...
RSpec.describe RAAF::Logger do
  ...
  describe "category-specific debug methods" do
    ...
    describe "#log_debug_context" do
      it "logs with context category" do
        instance.log_debug_context("Context updated", keys: %w[user session])

        expect(mock_logging).to have_received(:debug).with("Context updated", category: :context, keys: %w[user session])
      end
    end
  ...
  describe "#log_exception" do
    ...
    it "includes cause information when exception has a cause" do
      ...
      expect(mock_logging).to have_received(:error) do |_message, context|
