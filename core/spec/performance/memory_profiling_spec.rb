...
RSpec.describe "Memory Profiling", :performance do
  ...
  describe "Memory allocation patterns" do
    it "profiles agent creation memory usage" do
      ...
      objects_per_agent = report.total_allocated_objects / 100.0
    ...
    it "identifies memory hotspots in tool execution" do
      ...
      array_allocations = report.total_allocated_objects
      ...
      expect(string_allocations).to be > 0
    end
  end
  ...
  describe "Garbage collection impact" do
    ...
    it "profiles memory allocation types" do
      ...
      hash_allocations = report.total_allocated_objects - string_allocations.size
  ...
  describe "Memory benchmarking" do
    it "benchmarks memory usage across different operation types" do
      ...
      expect(tool_benchmark.allocated).to be > simple_benchmark.allocated
      expect(tool_benchmark.allocated).to be < simple_benchmark.allocated * 3  # Not more than 3x
      
      # Both should have minimal retained memory
      expect(simple_benchmark.retained).to be < 10_000   # Less than 10KB retained
      expect(tool_benchmark.retained).to be < 20_000     # Less than 20KB retained
    end
    ...
    it "benchmarks memory efficiency of different configurations" do
      ...
      complex_memory = memory do
        runner = RAAF::Runner.new(
          agent: base_agent, 
          config: complex_config,
          provider: mock_provider
        )
        runner.run("Complex test")
      end
      
      # Complex config should not dramatically increase memory usage
      memory_overhead = complex_memory.allocated - minimal_memory.allocated
      expect(memory_overhead).to be < 50_000  # Less than 50KB overhead
      
      # Retained memory should be similar
      retained_overhead = complex_memory.retained - minimal_memory.retained
