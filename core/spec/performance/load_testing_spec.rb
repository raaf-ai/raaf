    ...
    it "recovers gracefully from provider outages" do
      ...
      1000.times do |i|
        if i >= 200 && i < 300  # Outage from request 200-299
          mock_provider.add_error(RAAF::APIError.new("Provider unavailable"))
        else
          mock_provider.add_response("Available response #{i}")
        end
      end
      ...
      pre_outage_avg = response_times[0..199].sum / 200.0
      post_outage_avg = response_times[300..499].sum / 200.0
      
      # Post-outage performance should be similar to pre-outage
      expect(post_outage_avg).to be < (pre_outage_avg * 1.5)
    end
  end
