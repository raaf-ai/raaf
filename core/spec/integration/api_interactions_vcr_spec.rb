    ...
    context "basic completions" do
      ...
      it "handles streaming responses" do
        VCR.use_cassette("responses_api/streaming_completion") do
          ...
          expect(chunks.first).to have_key("output")
        end
      end
    end
    ...
    context "error handling" do
      ...
      it "handles rate limit errors" do
        VCR.use_cassette("responses_api/error_rate_limit") do
          # This cassette would need to be recorded when hitting rate limits
          # For now, we'll simulate by making rapid requests
          messages = [
            { role: "user", content: "Test message" }
          ]
          
          # In real scenario, this would trigger rate limit
          # For testing, we can mock the response
          allow(provider).to receive(:make_api_request).and_raise(
            RAAF::RateLimitError.new("Rate limit exceeded")
          )
          
          expect {
            provider.complete(messages: messages, model: "gpt-4o-mini")
    ...
    context "multi-turn conversations" do
      it "records full conversation flow" do
        VCR.use_cassette("responses_api/multi_turn_conversation") do
          ...
          expect(response2["output"]).to be_an(Array)
          
          # Should remember the name from context
          assistant_content2 = response2["output"]
            .select { |o| o["type"] == "message" }
            .map { |o| o["content"] }
            .flatten
            .select { |c| c && c["type"] == "text" }
            .map { |c| c["text"] }
            .join
          
          expect(assistant_content2.downcase).to include("alice")
        end
      end
    end
  end
    ...
    context "agent handoffs" do
      ...
      it "records handoff interactions" do
        VCR.use_cassette("runner/agent_handoff") do
          ...
          # Simulate a request that would trigger handoff
          result = handoff_runner.run(
          ...
          # Check if handoff occurred by examining agent transitions
          # This depends on the actual API behavior
        end
      end
    end
      ...
      it "records tool execution flow" do
        VCR.use_cassette("runner/tool_execution") do
          ...
          if tool_messages.any?
            expect(tool_messages.first[:content]).to match(/\d+/)
          end
    ...
    context "multi-agent research workflow" do
      ...
      it "records complete multi-agent workflow" do
        VCR.use_cassette("workflows/research_pipeline") do
          ...
          # Verify the workflow progressed through agents
          # The exact flow depends on model decisions
        end
      end
    end
