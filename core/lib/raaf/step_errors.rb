...
module RAAF
  ...
  module Errors
    ...
    class ModelBehaviorError < StepProcessingError
      ...
      def initialize(message, model_response: nil, **kwargs)
    ...
    class ToolExecutionError < StepProcessingError
      ...
      def initialize(message, tool: nil, tool_arguments: nil, original_error: nil, **kwargs)
        super(message, **kwargs)
        @tool = tool
    ...
    class HandoffError < StepProcessingError
      ...
      def initialize(message, source_agent: nil, target_agent: nil, handoff_data: nil, **kwargs)
        super(message, **kwargs)
    ...
    class ResponseProcessingError < StepProcessingError
      ...
      def initialize(message, response: nil, **kwargs)
        super(message, **kwargs)
        @response = response
      end
    end
    ...
    class CircularHandoffError < HandoffError
      ...
      def initialize(message, handoff_chain: nil, **kwargs)
        super(message, **kwargs)
    ...
    class MaxIterationsError < StepProcessingError
      ...
      def initialize(message, max_iterations: nil, current_iterations: nil, **kwargs)
        super(message, **kwargs)
  ...
  module ErrorHandling
    ...
    def self.with_error_handling(context: {}, agent: nil)
      ...
      if e.message.include?("undefined method") && e.message.include?("nil:NilClass")
        error = Errors::ModelBehaviorError.new(
          "Unexpected nil value in model response: #{e.message}",
          agent: agent
        )
      else
        error = Errors::AgentError.new(
    ...
    def self.safe_tool_execution(tool:, arguments:, agent:)
      with_error_handling(context: { tool: tool, arguments: arguments }, agent: agent) do
        yield
      end
    ...
    def self.validate_model_response(response, agent)
      ...
      if response[:output].nil? && response[:choices].nil? && response[:content].nil?
        raise Errors::ModelBehaviorError.new(
