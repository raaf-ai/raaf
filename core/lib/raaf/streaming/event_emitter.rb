  ...
  module Streaming
    ...
    class EventEmitter
        ...
        @mutex.synchronize do
          ...
          if @enable_wildcards
            @wildcard_listeners.each do |listener|
              listeners << listener if event =~ listener[:regex]
            end
          end
        end
      ...
      def add_to_history(event_data)
        ...
        return unless @event_history.size > @max_history_size
    ...
    class FilteredEventEmitter
      ...
      def on(event, &block)
        @emitter.on(event) do |data|
          block.call(data) if @filter.call(data)
        end
      end
      ...
      def emit(event, data = {})
        ...
        return unless @filter.call(event_data)
