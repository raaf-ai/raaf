...
module RAAF
  ...
  module Streaming
    ...
    class AsyncRunner
        ...
        while task_ids.any? && (Time.current - start_time) < timeout
          @mutex.synchronize do
            task_ids.each do |task_id|
              ...
              next unless task && task[:status] == :completed
      ...
      #
      def create_streaming_session(agent, **)
        AsyncStreamingSession.new(agent: agent, runner: self, **)
      end
      ...
      def shutdown
        ...
        @mutex.synchronize do
          @active_tasks.each_value do |task|
            task[:status] = :cancelled if task[:status] == :running
          end
        end
