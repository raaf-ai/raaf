    ...
    class Agent < RAAF::Agent
      ...
      def execute_tool_async(tool_name, **kwargs)
        ...
        if tool.respond_to?(:call_async)
          ...
          # Wrap tool in async block (handles both async and sync tools)
          Async do
            tool.call(**kwargs)
          end
        end
      ...
      def add_tool(tool)
        # Wrap callable objects in async-aware FunctionTool
        tool = AsyncFunctionTool.new(tool) if tool.is_a?(Proc) || tool.is_a?(Method) || (tool.respond_to?(:call) && !tool.respond_to?(:to_h))
