    ...
    class StreamingProvider
      ...
      def stream(messages, **, &)
        if supports_native_streaming?
          native_stream(messages, **, &)
        else
          simulated_stream(messages, **, &)
        end
      end
      ...
      def create_session(agent, **)
        StreamingSession.new(provider: self, agent: agent, **)
      end
      ...
      def simulated_stream(messages, **, &block)
        ...
        begin
          ...
          StreamProcessor.chunk_response(content, chunk_size: @config[:chunk_size]) do |chunk|
            ...
            sleep(@config[:simulation_delay]) if @config[:simulation_delay].positive?
          end
