  ...
  class BatchProcessor
    ...
    def wait_for_completion(batch_id, poll_interval: 30, max_wait_time: 3600)
      ...
      loop do
        ...
        # Check timeout
        raise BatchError, "Batch #{batch_id} timed out after #{max_wait_time} seconds" if Time.now - start_time > max_wait_time
    ...
    def download_file(file_id)
      ...
      raise BatchError, "Failed to download file: #{response.code} #{response.body}" unless response.is_a?(Net::HTTPSuccess)
