...
Complete guide for deploying RAAF (Ruby AI Agents Factory) applications to production.
...
export RAAF_ENVIRONMENT="production"
...
```bash
# Tracing
export RAAF_TRACE_BATCH_SIZE="100"
export RAAF_TRACE_FLUSH_INTERVAL="5"
...
ENV RAAF_ENVIRONMENT=production
...
# ecs-task-definition.json
{
  ...
  "containerDefinitions": [
    {
      "name": "app",
      "image": "your-ecr-repo/raaf-app:latest",
      "portMappings": [
        {
          "containerPort": 3000,
          "protocol": "tcp"
...
Description: 'RAAF (Ruby AI Agents Factory) Application'
...
spec:
  template:
    ...
    spec:
      ...
      timeoutSeconds: 300
      containers:
      - image: gcr.io/PROJECT-ID/raaf-app:latest
        ports:
        - containerPort: 3000
        env:
        - name: RAILS_ENV
...
heroku config:set RAAF_ENVIRONMENT=production
...
# Instrument runner
class InstrumentedRunner < RAAF::Runner
...
class LoggingRunner < RAAF::Runner
  def run(messages, **kwargs)
...
class BusinessRuleGuardrail < RAAF::Guardrails::BaseGuardrail
  def validate_input(input)
    ...
    if contains_pii?(input)
      raise RAAF::Guardrails::GuardrailError, "Input contains PII"
    end
    
    # Check for malicious patterns
    if contains_injection_patterns?(input)
      raise RAAF::Guardrails::GuardrailError, "Potential injection detected"
    end
  end
...
class PooledRunner < RAAF::Runner
...
class CachedRunner < RAAF::Runner
  def run(messages, **kwargs)
...
class CostControlledRunner < RAAF::Runner
...
class ResilientRunner < RAAF::Runner
  def run(messages, **kwargs)
    retry_count = 0
    max_retries = 3
...
class ProductionRunner < RAAF::Runner
  def run(messages, **kwargs)
    start_time = Time.current
