...
RSpec.describe "AiAgentDsl::VERSION" do
  subject { AiAgentDsl::VERSION }

  describe "version constant" do
    it "is defined" do
      expect(AiAgentDsl.const_defined?(:VERSION)).to be true
    end
  ...
  describe "module structure" do
    it "is defined within the AiAgentDsl module" do
      expect(AiAgentDsl.const_defined?(:VERSION)).to be true
    end

    it "can be accessed via the module" do
      expect(subject).to eq(AiAgentDsl.const_get(:VERSION))
    end
  end
  ...
  describe "integration with gem specification" do
    let(:gemspec_path) { File.expand_path("../../ai_agent_dsl.gemspec", __dir__) }
    let(:gemspec_content) { File.read(gemspec_path) }

    it "matches the version in the gemspec file" do
      # Extract version from gemspec
      expect(gemspec_content).to include("spec.version = AiAgentDsl::VERSION")
    end
  ...
  describe "constants immutability" do
    it "warns on version constant redefinition" do
      original_version = AiAgentDsl::VERSION
      begin
        expect do
          AiAgentDsl.const_set(:VERSION, "1.0.0")
        end.to output(/already initialized constant/).to_stderr
      ensure
        # Restore original version for other tests
        original_verbosity = $VERBOSE
        $VERBOSE = nil
        AiAgentDsl.const_set(:VERSION, original_version)
        $VERBOSE = original_verbosity
      end
    end

    it "maintains version constant visibility" do
      expect(AiAgentDsl.const_defined?(:VERSION, false)).to be true
    end
  end
  ...
  describe "version metadata" do
    it "can be used for runtime version checks" do
      version_check = ->(required_version) do
