...
RSpec.describe AiAgentDsl::Prompts::Base do
  ...
  describe "class DSL methods" do
    describe ".required" do
      it "sets required variables" do
        ...
        expect(prompt_class._required_variables).to eq([:document_name, :analysis_depth])
      end
    ...
    describe ".optional" do
      it "sets optional variables" do
        ...
        expect(prompt_class._optional_variables).to eq([:document_type, :location])
      end
    ...
    describe ".required with path" do
      ...
      it "prevents default values for required variables" do
        expect do
          Class.new(described_class) do
            required :document_type, path: [:document, :document_type], default: "Text"
          end
    ...
    describe ".optional with path" do
      ...
      it "supports default values" do
        prompt_class = Class.new(described_class) do
          optional :document_type, path: [:document, :document_type], default: "Text"
        end
  ...
  describe "initialization" do
    ...
    describe "with invalid context" do
      ...
      it "raises error for missing required variables" do
        ...
        expect do
          instance.validate!
        end.to raise_error(AiAgentDsl::Prompts::VariableContractError, /Missing required variables/)
      end
      ...
      it "raises error for unused variables in strict mode" do
        ...
        expect do
          instance.validate!
        end.to raise_error(AiAgentDsl::Prompts::VariableContractError, /Unused variables/)
      end
    end
    ...
    describe "with context mappings" do
      ...
      it "raises error for missing required context paths" do
        ...
        expect do
          instance.validate!
        end.to raise_error(AiAgentDsl::Prompts::VariableContractError, /Missing required context paths/)
      end
  ...
  describe "context-mapped variable access" do
    let(:context_mapped_class) do
      Class.new(described_class) do
        required :document_name, path: [:document, :name]
        optional :document_type, path: [:document, :document_type], default: "Unknown"
        optional :size, path: [:document, :metadata, :size]
      end
    end
    ...
    describe "with context mappings and regular variables" do
      let(:mixed_class) do
        Class.new(described_class) do
          ...
          required :mapped_field, path: [:nested, :field]
          contract_mode :strict
        end
      end
  ...
  describe "error handling" do
    describe "VariableContractError" do
      it "is a StandardError subclass" do
        expect(AiAgentDsl::Prompts::VariableContractError).to be < StandardError
      end

      it "can be raised with custom message" do
        expect do
          raise AiAgentDsl::Prompts::VariableContractError, "Custom error message"
        end.to raise_error(AiAgentDsl::Prompts::VariableContractError, "Custom error message")
      end
    end
    ...
    describe "context path resolution errors" do
      let(:error_class) do
        Class.new(described_class) do
          required :missing_field, path: [:nonexistent, :path]
        end
      end
      ...
      it "provides clear error messages for missing context paths" do
        ...
        expect do
          instance.validate!
        end.to raise_error(AiAgentDsl::Prompts::VariableContractError) do |error|
  ...
  describe "complex usage examples" do
    let(:comprehensive_prompt_class) do
      Class.new(described_class) do
        ...
        optional :document_type, path: [:document, :document_type], default: "Text"
        optional :size, path: [:document, :metadata, :size]
        optional :pages, path: [:document, :structure, :pages]
