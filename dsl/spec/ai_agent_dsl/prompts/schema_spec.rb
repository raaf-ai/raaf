...
RSpec.describe "Prompt Schema Support", type: :integration do
  ...
  class TestPromptWithSchema < AiAgentDsl::Prompts::Base
    requires :user_name
    optional :user_age
  ...
  class TestComplexSchemaPrompt < AiAgentDsl::Prompts::Base
    requires :analysis_target
    ...
    schema do
      field :results, type: :array, required: true do
        ...
        field :details, type: :object do
          field :category, type: :string, enum: ["high", "medium", "low"]
  ...
  class TestPromptWithoutSchema < AiAgentDsl::Prompts::Base
    requires :content
  ...
  class TestAgentWithPromptSchema < AiAgentDsl::Agents::Base
  ...
  class TestAgentWithConflictingSchema < AiAgentDsl::Agents::Base
    include AiAgentDsl::AgentDsl
  ...
  describe "schema inheritance and isolation" do
    class ParentPrompt < AiAgentDsl::Prompts::Base
      schema do
        field :parent_field, type: :string
      end
    end

    class ChildPrompt < ParentPrompt
      schema do
  ...
  describe "schema validation features" do
    ...
    it "supports enum validation for strings" do
      ...
      expect(category_field[:enum]).to eq(["high", "medium", "low"])
    end
