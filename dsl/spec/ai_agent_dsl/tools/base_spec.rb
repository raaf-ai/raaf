...
RSpec.describe AiAgentDsl::Tools::Base do
  ...
  describe "#execute_tool" do
    ...
    context "with tracing available" do
      ...
      it "uses tracing when available" do
        expect(OpenAIAgents.tracer).to receive(:custom_span).with(
          "tool_execution",
          {
      ...
      it "sets tracing attributes for successful execution" do
        span = double("Span")
        allow(OpenAIAgents.tracer).to receive(:custom_span).and_yield(span)
  ...
  describe "#handle_tool_error" do
    ...
    context "with Rails logger available" do
      it "logs error details to Rails logger" do
        ...
        expect(logger).to receive(:error).with(/Tool AiAgentDsl::Tools::Base execution failed: Test error message/)
