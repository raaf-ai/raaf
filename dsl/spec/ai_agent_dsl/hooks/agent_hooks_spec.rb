...
RSpec.describe AiAgentDsl::Hooks::AgentHooks do
  let(:agent_class) do
  ...
  describe "hook registration" do
    ...
    it "registers hooks for all supported event types" do
      agent_class.on_start { |agent| }
      agent_class.on_end { |agent, result| }
      agent_class.on_handoff { |from, to| }
      agent_class.on_tool_start { |agent, tool, params| }
      agent_class.on_tool_end { |agent, tool, params, result| }
      agent_class.on_error { |agent, error| }
  ...
  describe "combined hooks configuration" do
    ...
    it "includes only global hooks when no agent hooks" do
      ...
      AiAgentDsl::Hooks::RunHooks.on_agent_start(&global_proc)
  ...
  describe "hook validation" do
    ...
    it "raises error when both method and block provided" do
      expect do
        agent_class.on_start(:test_method) { |agent| }
