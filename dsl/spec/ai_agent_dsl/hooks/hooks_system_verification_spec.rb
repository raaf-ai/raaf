...
RSpec.describe "Hooks System Verification" do
  ...
  describe "hooks configuration and building" do
    it "successfully builds hooks configuration with both global and agent hooks" do
      ...
      AiAgentDsl::Hooks::RunHooks.on_tool_start { |_agent, tool, _params| execution_log << "global_tool_start:#{tool}" }
      AiAgentDsl::Hooks::RunHooks.on_tool_end { |_agent, tool, _params, _result| execution_log << "global_tool_end:#{tool}" }
      AiAgentDsl::Hooks::RunHooks.on_error { |_agent, error| execution_log << "global_error:#{error.class}" }
      AiAgentDsl::Hooks::RunHooks.on_handoff { |from, to| execution_log << "global_handoff:#{from}-#{to}" }
    ...
    it "executes tool hooks with parameters" do
      # Set up tool hooks
      AiAgentDsl::Hooks::RunHooks.on_tool_start { |_agent, tool, params| execution_log << "global_tool_start:#{tool}:#{params[:query]}" }
      AiAgentDsl::Hooks::RunHooks.on_tool_end { |_agent, tool, _params, result| execution_log << "global_tool_end:#{tool}:#{result[:status]}" }
    ...
    it "executes error hooks with error information" do
      # Set up error hooks
      AiAgentDsl::Hooks::RunHooks.on_error { |_agent, error| execution_log << "global_error:#{error.class}:#{error.message}" }
    ...
    it "executes handoff hooks with agent information" do
      ...
      AiAgentDsl::Hooks::RunHooks.on_handoff { |from, to| execution_log << "global_handoff:#{from.name}-#{to}" }
    ...
    it "supports multiple hooks of same type in correct order" do
      ...
      AiAgentDsl::Hooks::RunHooks.on_agent_start { |_agent| execution_log << "global_start_3" }
  ...
  describe "hooks system completeness" do
    it "provides complete coverage of all hook types" do
      ...
      expected_hook_types.each do |hook_type|
        AiAgentDsl::Hooks::RunHooks.public_send(hook_type) { |*_args| execution_log << "global_#{hook_type}" }
      end
