...
# Make sure the constant is available
require "raaf-dsl"

RSpec.describe RAAF::DSL::Agents::AgentDsl, :with_temp_files do
  ...
  describe "class methods" do
      ...
      context "when not set" do
        it "falls back to YAML config" do
          ...
          with_config_file(test_config) do |config_path|
            ...
            # Instead of relying on environment detection, stub the Config.model_for method directly
            # This eliminates the dependency on complex environment setup
            allow(RAAF::DSL::Config).to receive(:model_for).with("test_agent").and_return("gpt-4o-mini")
      ...
      context "when not set" do
        it "falls back to YAML config" do
          ...
          with_config_file(test_config) do |config_path|
            ...
            # Instead of relying on environment detection, stub the Config.max_turns_for method directly
            # This eliminates the dependency on complex environment setup
            allow(RAAF::DSL::Config).to receive(:max_turns_for).with("test_agent").and_return(2)
    ...
    describe ".uses_tool" do
      it "adds a tool to the configuration" do
        ...
        expect(tools_config.length).to eq(1)
      ...
      it "can add multiple tools" do
        ...
        tools_config = test_class._tools_config
        expect(tools_config.length).to eq(2)
        expect(tools_config.map { |t| t[:name] }).to eq(%i[web_search database_query])
      end
    end
    ...
    describe ".uses_tools" do
      it "adds multiple tools at once" do
        ...
        tools_config = test_class._tools_config
        expect(tools_config.length).to eq(3)
        expect(tools_config.map { |t| t[:name] }).to eq(%i[web_search database_query calculator])
      end
    end
    ...
    describe ".uses_tool_if" do
      it "adds tool when condition is true" do
        ...
        expect(test_class._tools_config.length).to eq(1)
      end
    ...
    describe ".handoff_to" do
      it "adds single handoff agent" do
        ...
        expect(handoffs.length).to eq(1)
    ...
    describe ".configure_handoffs" do
      it "configures multiple handoffs" do
        ...
        handoffs = test_class._agent_config[:handoff_agents]
        expect(handoffs.length).to eq(2)
      end
    end
    ...
    describe ".handoff_to_if" do
      it "adds handoff when condition is true" do
        ...
        expect(test_class._agent_config[:handoff_agents].length).to eq(1)
      end
    ...
    describe "workflow aliases" do
      %w[orchestrates discovery_workflow coordinates].each do |method|
    ...
    describe ".inferred_agent_name" do
      ...
      it "removes RAAF::DSL::Agents namespace" do
        test_class = Class.new(RAAF::DSL::Agents::Base) do
          include RAAF::DSL::Agents::AgentDsl
        end
        stub_const("RAAF::DSL::Agents::TestAgent", test_class)
  ...
  describe "instance methods" do
      ...
      context "with instruction template" do
        it "builds templated instructions" do
          test_class = create_test_agent_class do
            ...
            instruction_variables do
              content_name { context_hash.dig(:content, :name) }
            end
          end
    ...
    describe "#build_user_prompt" do
      ...
      context "without prompt class" do
        it "raises an error" do
          ...
          expect { instance.build_user_prompt }.to raise_error(RAAF::DSL::Error, /No prompt class configured/)
        end
      end
    end
    ...
    describe "#prompt_class_configured?" do
      ...
      it "returns true when inferred prompt class exists" do
        ...
        inferred_prompt_class = create_test_prompt_class("RAAF::DSL::Prompts::TestAgent")
    ...
    describe "#default_prompt_class" do
      it "infers prompt class from agent class name" do
        ...
        stub_const("RAAF::DSL::Agents::TestAgent", agent_class)

        # Create the expected prompt class
        prompt_class = create_test_prompt_class("RAAF::DSL::Prompts::TestAgent")
    ...
    describe "#handoffs" do
      ...
      it "passes context and processing_params to handoff agents" do
        ...
        expect(handoff_agent.context).to be_a(RAAF::DSL::ContextVariables)
        expect(handoff_agent.context.to_h).to eq(context)
  ...
  describe "helper classes" do
    describe "InstructionVariables" do
      let(:variables_builder) { RAAF::DSL::InstructionVariables.new }
    ...
    describe "SchemaBuilder" do
      let(:schema_builder) { RAAF::DSL::SchemaBuilder.new }
    ...
    describe "WorkflowBuilder" do
      let(:workflow_builder) { RAAF::DSL::WorkflowBuilder.new }
