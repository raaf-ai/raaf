...
RSpec.describe RAAF::DSL::ObjectProxy do
  ...
  describe "#initialize" do
    ...
    it "accepts configuration options" do
      product = TestProduct.new
      proxy = described_class.new(product, only: %i[id name], cache: false)

      expect(proxy.__options__).to eq(only: %i[id name], cache: false)
    end
  end
  ...
  describe "access control" do
    ...
    context "with :only option" do
      let(:proxy) { described_class.new(product, only: %i[id name]) }
  ...
  describe "edge cases" do
    ...
    it "handles respond_to_missing? correctly" do
      product = TestProduct.new
      proxy = described_class.new(product, only: %i[id name])
