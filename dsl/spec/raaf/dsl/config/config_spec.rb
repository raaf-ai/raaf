...
RSpec.describe RAAF::DSL::Config, :with_temp_files do
  ...
  before do
    ...
    described_class.instance_variable_set(:@raw_config, nil)

    # Also reset RAAF::DSL configuration to ensure clean state
    RAAF::DSL.instance_variable_set(:@configuration, nil)
    # Force new configuration with defaults
    RAAF::DSL.configuration

    # Hide Rails constant if it exists to ensure consistent test behavior
    # This prevents load_config from using Rails.root instead of configured path
    if defined?(Rails) && !Rails.is_a?(RSpec::Mocks::Double)
      @rails_was_defined = true
      @original_rails = Rails
      hide_const("Rails")
    else
      @rails_was_defined = false
    end
  end

  after do
    # Restore Rails if it was hidden
    stub_const("Rails", @original_rails) if @rails_was_defined && @original_rails
