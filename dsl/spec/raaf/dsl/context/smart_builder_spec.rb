...
RSpec.describe RAAF::DSL::Context::SmartBuilder do
  ...
  describe "#proxy" do
    ...
    it "applies access control with :only option" do
      builder = described_class.new
      builder.proxy(:product, product, only: %i[id name])
      context = builder.finalize
    ...
    it "applies access control with :except option" do
      builder = described_class.new
      builder.proxy(:product, product, except: %i[sensitive_data price])
      context = builder.finalize
  ...
  describe "#proxy_all" do
    it "proxies hash of objects" do
      builder = described_class.new
      builder.proxy_all({ product: product, company: company }, only: %i[id name])
      context = builder.finalize
    ...
    it "proxies array of objects with custom key" do
      ...
      builder = described_class.new
      builder.proxy_all(products, as: :products, only: %i[id name])
      context = builder.finalize
  ...
  describe "#requires" do
    it "validates required keys are present" do
      ...
      expect do
        builder.finalize
      end.to raise_error(ArgumentError, /Required context keys missing: company/)
    end
  ...
  describe "#validates" do
    it "validates type requirements" do
      ...
      expect do
        builder.finalize
      end.to raise_error(ArgumentError, /must be Integer but was String/)
    end
    ...
    it "validates range requirements" do
      ...
      expect do
        builder.finalize
      end.to raise_error(ArgumentError, /must be between 0 and 100/)
    end
    ...
    it "validates presence requirements" do
      ...
      expect do
        builder.finalize
      end.to raise_error(ArgumentError, /is required but missing/)
    end
    ...
    it "validates object attribute presence" do
      ...
      builder.validates(:product, presence: %i[name price])

      expect do
        builder.finalize
      end.to raise_error(ArgumentError, /missing required attributes: name/)
    end
    ...
    it "validates with custom proc" do
      ...
      expect do
        builder.finalize
      end.to raise_error(ArgumentError, /failed custom validation/)
    end
  end
