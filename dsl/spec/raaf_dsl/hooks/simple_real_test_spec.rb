...
RSpec.describe "Simple Real Hooks Test" do
  ...
  describe "real agent execution with hooks" do
    it "demonstrates hooks working with simulated agent run" do
      # Set up global hooks
      RAAF::DSL::Hooks::RunHooks.on_agent_start { |agent| execution_log << "global_start:#{agent.name}" }
      RAAF::DSL::Hooks::RunHooks.on_agent_end { |_agent, result| execution_log << "global_end:#{result[:success]}" }

      # Mock the agent run flow
      context = RAAF::DSL::ContextVariables.new({ user_input: "Hello" })
      ...
      expect(execution_log).to include("agent_start:RAAF::Agent")
    ...
    it "demonstrates hooks working with error scenarios" do
      # Set up error hooks
      RAAF::DSL::Hooks::RunHooks.on_error { |_agent, error| execution_log << "global_error:#{error.message}" }
    ...
    it "demonstrates hooks working with multiple agents" do
      ...
      execution_log_ref = execution_log
      second_agent_class = Class.new(RAAF::DSL::Agents::Base) do
        include RAAF::DSL::AgentDsl
        include RAAF::DSL::Hooks::AgentHooks
      ...
      # Set up global hooks
      RAAF::DSL::Hooks::RunHooks.on_agent_start { |agent| execution_log << "global_start:#{agent.name}" }
      RAAF::DSL::Hooks::RunHooks.on_agent_end { |agent, _result| execution_log << "global_end:#{agent.name}" }

      # Create and run first agent
      first_agent = agent_instance.create_agent
      first_runner = RAAF::Runner.new(agent: first_agent)
      ...
      second_runner = RAAF::Runner.new(agent: second_agent)
