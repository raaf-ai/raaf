...
RSpec.describe "Hooks Integration" do
  ...
  describe "hooks configuration building" do
    it "builds hooks configuration with global hooks only" do
      RAAF::DSL::Hooks::RunHooks.on_agent_start { |_agent| "global start" }
      RAAF::DSL::Hooks::RunHooks.on_agent_end { |_agent, _result| "global end" }
    ...
    it "builds configuration for all hook types" do
      RAAF::DSL::Hooks::RunHooks.on_agent_start { |_agent| "global" }
      RAAF::DSL::Hooks::RunHooks.on_agent_end { |_agent, _result| "global" }
      RAAF::DSL::Hooks::RunHooks.on_handoff { |_from, _to| "global" }
      RAAF::DSL::Hooks::RunHooks.on_tool_start { |_agent, _tool, _params| "global" }
      RAAF::DSL::Hooks::RunHooks.on_tool_end { |_agent, _tool, _params, _result| "global" }
      RAAF::DSL::Hooks::RunHooks.on_error { |_agent, _error| "global" }
  ...
  describe "combined hooks execution" do
    it "executes global hooks before agent-specific hooks" do
      ...
      RAAF::DSL::Hooks::RunHooks.on_agent_start { |_agent| execution_order << "global" }
      agent_class.on_start { |_agent| execution_order << "agent" }

      hooks_config = agent_instance.send(:build_hooks_config)
      hooks_config[:on_agent_start].call(test_agent)

      expect(execution_order).to eq(%w[global agent])
    end

    it "executes multiple global hooks in order, then agent hooks in order" do
      execution_order = []

      RAAF::DSL::Hooks::RunHooks.on_agent_start { |_agent| execution_order << "global1" }
      RAAF::DSL::Hooks::RunHooks.on_agent_start { |_agent| execution_order << "global2" }
      ...
      expect(execution_order).to eq(%w[global1 global2 agent1 agent2])
    end

    it "handles errors in global hooks gracefully" do
      execution_order = []

      RAAF::DSL::Hooks::RunHooks.on_agent_start { |_agent| raise "Global error" }
      agent_class.on_start { |_agent| execution_order << "agent" }

      # Mock error handling
      allow(RAAF::DSL::Hooks::RunHooks).to receive(:warn)
  ...
  describe "hook wrapper creation" do
    it "creates global hook wrapper that calls RunHooks" do
      RAAF::DSL::Hooks::RunHooks.on_agent_start { |_agent| "executed" }

      wrapper = agent_instance.send(:create_hook_wrapper, :on_agent_start, :global)

      expect(RAAF::DSL::Hooks::RunHooks).to receive(:execute_hooks).with(:on_agent_start, test_agent)
    ...
    it "handles hook wrapper creation for agents without AgentHooks" do
      ...
      basic_agent_class = Class.new(RAAF::DSL::Agents::Base) do
        include RAAF::DSL::AgentDsl
  ...
  describe "real-world usage scenarios" do
    ...
    it "supports method-based and block-based hooks mixed together" do
      ...
      # Global hooks
      RAAF::DSL::Hooks::RunHooks.on_agent_start { |_agent| execution_log << "global_block" }
      ...
      expect(execution_log).to eq(%w[global_block agent_block])
  ...
  describe "agent.run() execution integration" do
    ...
    let(:mock_openai_agent) { instance_double(RAAF::Agent) }
    ...
    before do
      ...
      allow(agent_instance).to receive_messages(build_user_prompt_with_context: "test prompt",
                                                create_runner: mock_runner)
    ...
    it "verifies tool hooks are called during agent execution" do
      ...
      expect(execution_log).to eq(%w[global_tool_start agent_tool_start global_tool_end agent_tool_end])
      expect(result).to eq(mock_result)
    end
    ...
    it "verifies error hooks are called during agent execution failures" do
      ...
      expect(execution_log).to eq(%w[global_error agent_error])
    ...
    it "verifies handoff hooks are called during agent handoffs" do
      ...
      expect(execution_log).to eq(%w[global_handoff agent_handoff])
      expect(result).to eq(mock_result)
    end
  end

  describe "end-to-end RAAF framework integration" do
    let(:real_openai_agent) { instance_double(RAAF::Agent) }
    let(:execution_log) { [] }
    let(:mock_runner) { instance_double(RAAF::Runner) }
    let(:mock_result) { { success: true, data: "test result" } }
    ...
    it "verifies framework hooks execute in correct lifecycle order" do
      ...
      allow(mock_runner).to receive(:run) do |_prompt|
        ...
        # This simulates the actual RAAF framework behavior
    ...
    it "verifies framework hooks are resilient to errors" do
      ...
      expect(execution_log).to eq(%w[agent_start_success global_end_success agent_end_success])
      expect(result).to eq(mock_result)
    end
  end
end
