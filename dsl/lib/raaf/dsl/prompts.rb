...
module RAAF
  module DSL
    ...
    class Prompt
      ...
      #   )
      def initialize(id:, version: nil, variables: nil, messages: nil, schema: nil, metadata: nil)
        @id = id
        @version = version
        @variables = variables || {}
        @messages = messages
      ...
      ##
      # Resolve a prompt specification using the configured resolvers
      #
      # @param prompt_spec [Object] the prompt specification (String, Class, Hash, etc.)
      # @param context [Hash] runtime context for prompt generation
      # @return [Prompt, nil] resolved prompt or original if already a Prompt
      #
      # @example Resolve from file
      #   prompt = Prompt.resolve("customer_service.md")
      #
      # @example Resolve from Phlex class
      #   prompt = Prompt.resolve(CustomerServicePrompt)
      #
      # @example Resolve with context
      #   prompt = Prompt.resolve("template.md.erb", user_name: "John", date: Date.today)
      #
      def self.resolve(prompt_spec, context = {})
        return prompt_spec if prompt_spec.is_a?(Prompt)

        # Ensure default configuration is applied
        DSL.prompt_configuration.apply! if DSL.prompt_resolvers.resolvers.empty?

        # Attempt resolution
        DSL.prompt_resolvers.resolve(prompt_spec, context)
      end
