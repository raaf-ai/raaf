...
# all execution to the openai-agents-ruby library. It does not contain any execution
...
#   result = agent.run  # Delegates to openai-agents-ruby
#
# @see AiAgentDsl::AgentDsl For DSL configuration methods
# @see AiAgentDsl::Config For configuration management
# @since 0.1.0
#
class AiAgentDsl::Agents::Base
  ...
  alias context_variables context
  ...
  def initialize(context: nil, context_variables: nil, processing_params: {}, debug: nil)
    ...
    @context = case context_param
               when AiAgentDsl::ContextVariables
                 context_param
               when Hash
                 AiAgentDsl::ContextVariables.new(context_param, debug: @debug_enabled)
               when nil
                 AiAgentDsl::ContextVariables.new({}, debug: @debug_enabled)
               else
  ...
  #
  # This method creates an OpenAIAgents::Agent instance configured with all
  ...
  #
  # @return [OpenAIAgents::Agent] Configured OpenAI agent instance ready for execution
  #
  def create_agent
  ...
  # Runs the agent by delegating to openai-agents-ruby
  #
  # This method is pure delegation - it creates an OpenAI agent with the DSL
  # configuration and lets openai-agents-ruby handle all execution logic.
  #
  ...
  # @return [Hash] Result hash from openai-agents-ruby execution
  #
  def run(context: nil, input_context_variables: nil, stop_checker: nil)
    ...
    # Create OpenAI Agents runner and delegate execution
    runner_params = { agent: openai_agent }
    runner_params[:stop_checker] = stop_checker if stop_checker

    runner = OpenAIAgents::Runner.new(**runner_params)

    # Pure delegation to openai-agents-ruby
  ...
  # OpenAI Agents compatibility methods
  def instructions
    build_instructions
  end
  ...
  def tools
    if respond_to?(:build_tools_from_config, true)
  ...
  def resolve_run_context(context)
    ...
    when AiAgentDsl::ContextVariables
      context
    when Hash
      AiAgentDsl::ContextVariables.new(context, debug: @debug_enabled)
    when nil
      @context
    else
      raise ArgumentError, "context must be ContextVariables instance, Hash, or nil"
  ...
  # Transform OpenAI Agents result to expected DSL format
  ...
  def create_openai_agent_instance
    ...
    OpenAIAgents::Agent.new(**agent_params)
  end

  # Build hooks configuration for OpenAI Agents
  ...
  alias create_agent_instance create_openai_agent_instance
end
