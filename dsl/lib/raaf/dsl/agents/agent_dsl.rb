    ...
    module Agents
      ...
      module AgentDsl
        ...
        def handle_no_executable_method(tool_name, tool_instance)
          RAAF.logger.warn "⚠️ No executable method found for tool #{tool_name}. Available methods: #{tool_instance.methods(false)}"
          nil
        end
        ...
        def validate_tool_parameters(tool_name, tool_parameters)
          unless tool_parameters.is_a?(Hash) && tool_parameters[:type] == "object"
            RAAF.logger.warn "⚠️ Tool #{tool_name} has invalid parameter schema"
          end
        end

        def create_raaf_function_tool(tool_name, tool_description, tool_parameters, tool_proc)
