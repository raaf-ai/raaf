...
# Unified logging framework that integrates with the RAAF library
#
# This module provides a consistent logging interface across the AI Agent DSL framework,
# leveraging the RAAF tracing capabilities when available and falling back
# to standard Rails logging when not.
#
# Key features:
# - Structured logging with consistent format
# - Integration with RAAF tracer for distributed tracing
  ...
  module DSL
    ...
    module Logging
      ...
      module ClassMethods
        ...
        # @option options [Boolean] :enable_tracing Enable RAAF tracing (default: auto-detect)
        ...
        LOG_LEVELS.each_key do |level|
          define_method("log_#{level}") do |message, metadata = {}|
            RAAF::DSL::Logging.log(level, message, metadata.merge(
      ...
      module InstanceMethods
        ...
        LOG_LEVELS.each_key do |level|
          define_method("log_#{level}") do |message, metadata = {}|
            ...
            RAAF::DSL::Logging.log(level, message, enriched_metadata)
          end
        end
        ...
        def tracing_enabled?
          ...
          if config[:enable_tracing].nil?
            # Auto-detect based on RAAF availability and Rails environment
        ...
        # Check if RAAF tracer is available
        def openai_tracer_available?
          defined?(OpenAIAgents) && RAAF.respond_to?(:tracer)
        end
