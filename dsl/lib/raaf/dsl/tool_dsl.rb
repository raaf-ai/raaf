...
# Provides a clean, readable interface for configuring tools with
# parameters, validation, and metadata. This DSL is configuration-only
# and generates tool definitions that are passed to the OpenAI Agents
...
module AiAgentDsl::ToolDsl
  ...
  class_methods do
    ...
    #
    # @return [Hash] Tool configuration for OpenAI Agents framework
    #
  ...
  # Build tool configuration for OpenAI Agents framework
  #
  ...
  def build_parameter_schema(config)
    ...
    # Handle array items
    if config[:type] == :array && config[:items_type]
      schema[:items] = { type: openai_type(config[:items_type]) }
    end
