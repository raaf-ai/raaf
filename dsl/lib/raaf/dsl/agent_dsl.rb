  ...
  module DSL
    ...
    module AgentDsl
      ...
      class_methods do
        ...
        def infer_default_prompt_class
          ...
          prompt_class_name = convert_agent_to_prompt_class_name(agent_class_name)

          prompt_class_name.constantize
        rescue NameError => e
          raise NameError, build_prompt_class_error_message(agent_class_name, prompt_class_name, e)
        end

        private

        def convert_agent_to_prompt_class_name(agent_class_name)
          ...
          return prompt_class_name if prompt_class_name.include?("::Prompts::")

          handle_non_standard_namespace(agent_class_name)
        end

        def handle_non_standard_namespace(agent_class_name)
          if agent_class_name.include?("::")
            handle_namespaced_class(agent_class_name)
          else
            # No namespace - simple conversion
            agent_class_name.gsub(/Agent$/, "Prompt")
          end
        end

        def handle_namespaced_class(agent_class_name)
          parts = agent_class_name.split("::")
          return agent_class_name unless parts.length >= 2
        ...
        def build_prompt_class_error_message(agent_class_name, prompt_class_name, original_error)
          "No prompt class found for #{agent_class_name}. " \
      ...
      # Alias for RAAF Runner compatibility
      def model
        model_name
      end
      ...
      def tools
        @tools ||= begin
          ...
          log_final_tools(built_tools) if should_log_tools?
          built_tools
        end
      end
      ...
      # @raise [RAAF::DSL::Error] If no prompt class is configured or methods are missing
      #
      # @example
      ...
      def build_instructions
        ...
        unless prompt_class_configured?
          raise RAAF::DSL::Error, "No prompt class configured for #{self.class.name}. " \
      ...
      #
      # @return [RAAF::DSL::Prompts::Base] The configured prompt instance
      # @raise [RAAF::DSL::Error] If prompt class validation fails
      #
      # @example
      #   agent = MyAgent.new(
      ...
      #
      # Supported patterns:
      # - RAAF::DSL::Agents::MyAgent -> RAAF::DSL::Prompts::MyAgent
      ...
      #
      # @example
      #   class RAAF::DSL::Agents::Company::Discovery < Base
      #     # Will try to find RAAF::DSL::Prompts::Company::Discovery
      #   end
      #
      def default_prompt_class
        @default_prompt_class ||= begin
          ...
          # Support both RAAF::DSL::Agents:: and any ::Agents:: namespace
      ...
      #
      # @return [Array<RAAF::Agent>] Array of OpenAI agent instances for handoffs
      #
      # @example
      #   class OrchestratorAgent < RAAF::DSL::Agents::Base
      #     include RAAF::DSL::AgentDsl
      #
      ...
      #   orchestrator.handoffs # => [RAAF::Agent, RAAF::Agent]
      #
      def handoffs
      ...
      def extract_tool_name(tool_def, tool_instance)
        tool_def[:function]&.[](:name) || tool_def[:name] || tool_instance.tool_name
      end

      def extract_tool_description(tool_def)
        tool_def[:function]&.[](:description) || tool_def[:description] || "A function tool"
      end

      def extract_tool_parameters(tool_def)
        tool_def[:function]&.[](:parameters) || tool_def[:parameters] || {}
      end

      def log_tool_definition(tool_name, tool_def)
        RAAF::Logging.debug("📋 Tool definition from DSL for #{tool_name}:", category: :tools)
        RAAF::Logging.debug("   Full definition: #{tool_def.inspect}", category: :tools)
      end

      def find_executable_method(tool_name, tool_instance)
        return tool_name.to_sym if tool_name && tool_instance.respond_to?(tool_name.to_sym)
        return :execute_tool if tool_instance.respond_to?(:execute_tool)
        return :call if tool_instance.respond_to?(:call)

        # Try to find any method that might be the main execution method
        possible_methods = %i[execute run perform search fetch]
        possible_methods.find { |m| tool_instance.respond_to?(m) }
      end

      def create_tool_proc(tool_instance, method_to_call)
        proc do |**params|
          if keyword_arguments_supported?(tool_instance, method_to_call)
      ...
      def keyword_arguments_supported?(tool_instance, method_to_call)
        tool_instance.method(method_to_call).parameters.any? { |type, _| %i[keyreq key].include?(type) }
      end

      def validate_tool_parameters(tool_name, tool_parameters)
        if tool_parameters.nil? || tool_parameters.empty?
          RAAF::Logging.warn "⚠️ WARNING: Tool #{tool_name} has no parameters defined!"
        elsif !tool_parameters.is_a?(Hash)
          RAAF::Logging.warn "⚠️ WARNING: Tool #{tool_name} parameters are not a Hash: " \
                             "#{tool_parameters.class}"
        else
          log_valid_parameters(tool_name, tool_parameters)
          validate_array_parameters(tool_parameters)
        end
      end

      def log_valid_parameters(_tool_name, tool_parameters)
        RAAF::Logging.debug("✅ Parameters schema looks valid: type=#{tool_parameters[:type]}, " \
                            "properties=#{tool_parameters[:properties]&.keys}", category: :tools)
      end

      def validate_array_parameters(tool_parameters)
        tool_parameters[:properties]&.each do |prop_name, prop_def|
          next unless prop_def[:type] == "array"

          RAAF::Logging.debug("🔍 ARRAY PROPERTY DEBUG for #{prop_name}:", category: :tools)
          RAAF::Logging.debug("    Full property definition: #{prop_def.inspect}", category: :tools)
          RAAF::Logging.debug("    Has items property: #{prop_def.key?(:items)}", category: :tools)
          RAAF::Logging.debug("    Items value: #{prop_def[:items].inspect}", category: :tools) if prop_def[:items]

          if prop_def[:items].nil?
            RAAF::Logging.error("🚨 FOUND THE BUG! Array property '#{prop_name}' has nil items - " \
      ...
      def create_openai_function_tool(tool_name, tool_description, tool_parameters, tool_proc)
        log_tool_creation(tool_name, tool_parameters)

        openai_tool = ::RAAF::FunctionTool.new(
          tool_proc,
          name: tool_name,
        ...
        log_final_tool(tool_name, openai_tool)
        openai_tool
      end

      def log_tool_creation(tool_name, tool_parameters)
        RAAF::Logging.debug("🛠️ Creating OpenAI tool: #{tool_name}", category: :tools)
        RAAF::Logging.debug("📝 Parameters schema: #{tool_parameters.inspect}", category: :tools)
      end

      def log_final_tool(tool_name, openai_tool)
        RAAF::Logging.debug("📤 Final OpenAI tool created for #{tool_name}:", category: :tools)
        if openai_tool.respond_to?(:definition)
          RAAF::Logging.debug("    Tool definition: #{openai_tool.definition.inspect}", category: :tools)
        elsif openai_tool.respond_to?(:to_hash)
          RAAF::Logging.debug("    Tool hash: #{openai_tool.to_hash.inspect}", category: :tools)
        else
          RAAF::Logging.debug("    Tool object: #{openai_tool.inspect}", category: :tools)
        end
      end

      def log_no_executable_method(tool_name, tool_instance)
        return unless should_log_tools?

        RAAF::Logging.warn "No executable method found for tool: #{tool_name}"
        RAAF::Logging.warn "Available methods: #{tool_instance.methods(false).sort}"
      end

      def should_log_tools?
        # Use RAAF's unified logging system which handles Rails integration automatically
        RAAF::Logging.configuration.debug_enabled?(:tools)
      end

      def log_final_tools(built_tools)
        RAAF::Logging.info "🎯 Final tools for #{self.class.name}:"
        built_tools.each_with_index do |tool, idx|
          log_tool_info(tool, idx)
        end
      end

      def log_tool_info(tool, idx)
        RAAF::Logging.info "   Tool #{idx + 1}: #{tool.class.name}"
        return unless tool_has_required_methods?(tool)

        log_basic_tool_info(tool)
        log_tool_parameters(tool.parameters) if tool_has_parameters?(tool)
      end

      def tool_has_required_methods?(tool)
        tool.respond_to?(:name) && tool.respond_to?(:parameters)
      end

      def log_basic_tool_info(tool)
        RAAF::Logging.info "     Name: #{tool.name}"
        RAAF::Logging.info "     Parameters type: #{tool.parameters.class.name}"
        RAAF::Logging.info "     Parameters present: #{!tool.parameters.nil? && !tool.parameters.empty?}"
      end

      def tool_has_parameters?(tool)
        tool.parameters && tool.parameters[:properties]
      end

      def log_tool_parameters(parameters)
        RAAF::Logging.info "     Properties: #{parameters[:properties].keys}"
        RAAF::Logging.info "     Required: #{parameters[:required]}"
      end
      ...
      def expected_prompt_class_name
        ...
        # E.g., RAAF::DSL::Agents::MyAgent -> RAAF::DSL::Prompts::MyAgent
        # E.g., Ai::Agents::MyAgent -> Ai::Prompts::MyAgent
        agent_class_name = self.class.name

        if agent_class_name.start_with?("RAAF::DSL::Agents::")
          agent_class_name.sub("RAAF::DSL::Agents::", "RAAF::DSL::Prompts::")
        elsif agent_class_name.include?("::Agents::")
      ...
      def validate_prompt_methods!
        validate_system_method_exists
        validate_user_method_exists
        validate_system_method_arity
        validate_user_method_arity
      end

      def validate_system_method_exists
        return if prompt_instance.respond_to?(:system)

        raise RAAF::DSL::Error, "Prompt class #{prompt_instance.class.name} must implement 'system' method"
      end

      def validate_user_method_exists
        return if prompt_instance.respond_to?(:user)

        raise RAAF::DSL::Error, "Prompt class #{prompt_instance.class.name} must implement 'user' method"
      end

      def validate_system_method_arity
        return if prompt_instance.method(:system).arity <= 0

        raise RAAF::DSL::Error,
              "Prompt class #{prompt_instance.class.name} 'system' method must accept zero arguments " \
              "or have default parameters"
      end

      def validate_user_method_arity
        return if prompt_instance.method(:user).arity <= 0

        raise RAAF::DSL::Error,
              "Prompt class #{prompt_instance.class.name} 'user' method must accept zero arguments " \
              "or have default parameters"
      end
      ...
      def process_dsl_tool(tool_instance)
        ...
        log_tool_definition(tool_name, tool_def)
      ...
      def handle_no_executable_method(tool_name, tool_instance)
        log_no_executable_method(tool_name, tool_instance)
        tool_instance
      end
      ...
      def build_handoffs_from_config
        ...
        handoff_agent_configs.map do |handoff_config|
          dsl_agent_instance = create_dsl_agent_instance(handoff_config)
          convert_to_openai_agent(dsl_agent_instance)
        end
      end

      def create_dsl_agent_instance(handoff_config)
        if handoff_config.is_a?(Hash)
          create_agent_with_options(handoff_config)
        else
          # Direct agent class instantiation
          handoff_agent_class = handoff_config
          handoff_agent_class.new(context: @context, processing_params: @processing_params)
        end
      end

      def create_agent_with_options(handoff_config)
      ...
      def convert_to_openai_agent(dsl_agent_instance)
        if dsl_agent_instance.respond_to?(:create_agent_with_context)
          openai_agent = dsl_agent_instance.create_agent_with_context(
            dsl_agent_instance.instance_variable_get(:@context)
          )
          ...
          # Fallback for direct agent instances
    ...
    class SchemaBuilder
      ...
      def field(name, type:, required: false, **options, &block)
        ...
        handle_nested_field(field_schema, type, options, &block) if block_given?
        add_field_to_schema(name, field_schema, required)
      end

      private

      def handle_nested_field(field_schema, type, options, &block)
        case type
        when :object
          handle_object_field(field_schema, &block)
        when :array
          handle_array_field(field_schema, options, &block)
        end
      end

      def handle_object_field(field_schema, &block)
      ...
      def handle_array_field(field_schema, options, &block)
        if options[:items_type]
          field_schema[:items] = build_field_schema(options[:items_type], options)
        else
          handle_array_of_objects(field_schema, &block)
        end
      end

      def handle_array_of_objects(field_schema, &block)
        nested_builder = SchemaBuilder.new
        nested_builder.instance_eval(&block)
      ...
      def add_field_to_schema(name, field_schema, required)
      ...
      def build_field_schema(type, options)
        ...
        add_type_specific_constraints(schema, type, options)
        add_common_attributes(schema, options)
        schema
      end

      def add_type_specific_constraints(schema, type, options)
        case type
        when :integer
          add_integer_constraints(schema, options)
        when :string
          add_string_constraints(schema, options)
        when :array
          add_array_constraints(schema, options)
        end
      end

      def add_integer_constraints(schema, options)
      ...
      def add_string_constraints(schema, options)
      ...
      def add_array_constraints(schema, options)
      ...
      def add_common_attributes(schema, options)
