...
module AiAgentDsl::Prompts
  ...
  #   class CompanyEnrichment < AiAgentDsl::Prompts::Base
  ...
  #   class DocumentAnalysis < AiAgentDsl::Prompts::Base
  ...
  #   end
  #
  # @see AiAgentDsl::AgentDsl For integration with agents
  # @since 0.1.0
  #
  class Base
    ...
    def self.requires(*variables)
      required(*variables)
    end

    def self.contract_mode(mode)
      unless [:strict, :warn, :lenient].include?(mode)
    ...
    #   class CompanyAnalysis < AiAgentDsl::Prompts::Base
    #     schema do
    ...
    def self.schema(&block)
      if block_given?
        # Import SchemaBuilder from AiAgentDsl module
        schema_builder_class = const_get("::AiAgentDsl::SchemaBuilder")
      ...
      if value.nil? && default_value.nil? && mapping[:required]
        ...
        if @context
          ...
          if path.first && actual_context.key?(path.first)
            ...
            error_msg += "\nAvailable at #{path.first}: #{first_level.keys.inspect if first_level.respond_to?(:keys)}"
          end
        end
    ...
    def validate_variable_contract!
      ...
      return unless [:strict, :warn].include?(self.class._contract_mode) && declared_variables.any?
    ...
    def validate_context_mapped_variables!
      ...
      self.class._context_mappings.each do |variable_name, mapping|
        ...
        missing_paths << "#{variable_name} (context path: #{path.join(' -> ')})" if value.nil? && mapping[:default].nil?
      end
