...
#   class MarketResearchAgent < RAAF::DSL::Agents::Base
...
#   config = RAAF::DSL::Config.for_agent("market_research_agent")
#
#   # Get model for agent in current environment
#   model = RAAF::DSL::Config.model_for("market_research_agent")
#
#   # Get global configuration
#   global_config = RAAF::DSL::Config.global
#
# @see RAAF::DSL::AgentDsl For automatic configuration integration
# @since 0.1.0
#
module RAAF
      ...
      class << self
        ...
        def max_turns_for(agent_name, environment: current_environment)
          for_agent(agent_name, environment: environment)["max_turns"] ||
            RAAF::DSL.configuration.default_max_turns
        end
        ...
        def model_for(agent_name, environment: current_environment)
          for_agent(agent_name, environment: environment)["model"] ||
            RAAF::DSL.configuration.default_model
        end
        ...
        def temperature_for(agent_name, environment: current_environment)
          for_agent(agent_name, environment: environment)["temperature"] ||
            RAAF::DSL.configuration.default_temperature
        end
