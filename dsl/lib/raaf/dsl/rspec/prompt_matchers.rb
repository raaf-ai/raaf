...
#
# @example Basic usage
#   require 'ai_agent_dsl/rspec/prompt_matchers'
#
...
#
# @since 0.1.0
module AiAgentDsl::RSpec::PromptMatchers
  ...
  if defined?(::RSpec)
    ...
    ::RSpec::Matchers.define :include_prompt_content do |*expected_content|
      ...
      def render_prompts(prompt)
        ...
        raise ::RSpec::Expectations::ExpectationNotMetError,
          "Prompt validation failed: #{e.message}. Consider using .with_context() to provide required variables."
      end
      ...
      def build_failure_message(expected_content, rendered_content, inclusion:)
        ...
        if inclusion
          ...
          lines << "Unexpected content: #{@unexpected_content.map(&:inspect).join(', ')}" if @unexpected_content&.any?
        else
    ...
    ::RSpec::Matchers.define :have_prompt_context_variable do |variable_name|
      ...
      failure_message do
        if @contract_error
          "Expected prompt to have context variable #{@variable_name.inspect}, but got contract error: #{@contract_error.message}"
        elsif @expected_value
          "Expected context variable #{@variable_name.inspect} to have value #{@expected_value.inspect}, but got #{@actual_value.inspect}"
        else
