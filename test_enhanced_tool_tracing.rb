...
tracer.add_processor(OpenAIAgents::Tracing::RailsProcessor.new) if defined?(OpenAIAgents::Tracing::RailsProcessor)

# Add a custom processor to capture tool details
class DetailedToolProcessor
  def on_span_start(span)
    return unless span.kind == :tool
  ...
  def on_span_end(span)
    ...
    puts "  Function: #{span.attributes["function.name"]}" if span.attributes["function.name"]

    puts "  Input: #{span.attributes["function.input"].inspect}" if span.attributes["function.input"]
    ...
    puts "  Has Results: #{span.attributes["function.has_results"]}" if span.attributes["function.has_results"]

    puts "  Search Query: #{span.attributes["web_search.query"]}" if span.attributes["web_search.query"]
