  ...
  module Adapters
    ...
    class InMemoryAdapter
      ...
      def search(query_embedding, k: 5, namespace: nil, filter: nil)
        ...
        if filter
          filtered = filtered.select { |r| match_filter(r[:metadata], filter) }
        end
    ...
    class PgVectorAdapter
      ...
      def stats(namespace: nil)
        ...
        with_connection do |conn|
          ...
          result.map { |row| [row["namespace"], row["count"].to_i] }.to_h
        end
      end
      ...
      def import_records(records)
        ...
        all_records = []
        records.each do |namespace, namespace_records|
          namespace_records.each do |_, record|
            all_records << record
          end
        end
