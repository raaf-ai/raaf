    ...
    module RailsIntegrations
      ...
      module ConsoleHelpers
        ...
        def recent_traces(limit: 10)
          ...
          OpenAIAgents::Tracing::Trace.recent.limit(limit).includes(:spans)
        end
        ...
        def traces_for(workflow_name, limit: 10)
          ...
          OpenAIAgents::Tracing::Trace.by_workflow(workflow_name)
        ...
        def failed_traces(limit: 10)
          ...
          OpenAIAgents::Tracing::Trace.failed.recent.limit(limit).includes(:spans)
        end
        ...
        def slow_spans(threshold: 1000, limit: 20)
          return [] unless defined?(OpenAIAgents::Tracing::Span)

          OpenAIAgents::Tracing::Span.slow(threshold)
        ...
        def error_spans(limit: 20)
          ...
          OpenAIAgents::Tracing::Span.errors.recent.limit(limit).includes(:trace)
        end
        ...
        def span(span_id)
          return nil unless defined?(OpenAIAgents::Tracing::Span)

          OpenAIAgents::Tracing::Span.find_by(span_id: span_id)
        end
        ...
        def performance_stats(timeframe: 24.hours)
          return puts "Tracing models not available" unless defined?(OpenAIAgents::Tracing::Trace)

          start_time = timeframe.ago
          end_time = Time.current

          traces = OpenAIAgents::Tracing::Trace.within_timeframe(start_time, end_time)
          spans = OpenAIAgents::Tracing::Span.within_timeframe(start_time, end_time)
      ...
      class RakeTasks
        ...
        def self.cleanup_old_traces(older_than: 30.days)
          return 0 unless defined?(OpenAIAgents::Tracing::Trace)

          deleted_count = OpenAIAgents::Tracing::Trace.cleanup_old_traces(older_than: older_than)
        ...
        def self.performance_report(timeframe: 24.hours)
          ...
          # Overall stats
          stats = OpenAIAgents::Tracing::Trace.performance_stats(timeframe: start_time..end_time)
          ...
          # Top workflows
          puts "\n--- Top Workflows ---"
          top_workflows = OpenAIAgents::Tracing::Trace.top_workflows(limit: 10, timeframe: start_time..end_time)
          top_workflows.each do |workflow|
            puts "#{workflow[:workflow_name]}: #{workflow[:trace_count]} traces (#{workflow[:success_rate]}% success)"
          end

          # Error analysis
          error_analysis = OpenAIAgents::Tracing::Span.error_analysis(timeframe: start_time..end_time)
...
if defined?(Rails::Console)
  Rails::Console.class_eval do
    def start_with_tracing_helpers
      ...
      IRB::ExtendCommandBundle.include(OpenAIAgents::Tracing::RailsIntegrations::ConsoleHelpers)
