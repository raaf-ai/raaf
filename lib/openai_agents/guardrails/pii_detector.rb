    ...
    class PIIDetector < Base
      ...
      PII_PATTERNS = {
        ...
        drivers_license: {
          ...
          confidence: 0.6,
          validator: ->(match) { match.length >= 6 && match.length <= 10 }
        },
        passport: {
        ...
        bank_account: {
          ...
          confidence: 0.5,
          validator: ->(match) { match.length >= 8 && match.length <= 17 }
        },
        date_of_birth: {
        ...
        zip_code: {
          ...
          name: "ZIP Code",
          confidence: 0.5,
          validator: ->(match) { match.gsub(/\D/, "").length == 5 || match.gsub(/\D/, "").length == 9 }
        },
        ...
        PII_PATTERNS.merge(@custom_patterns).each do |type, config|
          ...
          matches.each do |match|
            ...
            if config[:confidence] < 0.6
              next unless context_suggests_pii?(text, match_str, type)
            end
    ...
    class FinancialPIIDetector < PIIDetector
      FINANCIAL_PATTERNS = {
        ...
        bitcoin_address: {
          ...
          confidence: 0.9,
          validator: ->(match) { match.length >= 26 && match.length <= 35 }
        }
      }.freeze
