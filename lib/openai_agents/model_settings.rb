module OpenAIAgents
  ...
  # == Basic Usage
  #
  #   settings = OpenAIAgents::ModelSettings.new(
  #     temperature: 0.7,
  #     max_tokens: 2000,
  #     top_p: 0.9
  #   )
  ...
  class ModelSettings
    ...
    def validate_temperature
      ...
      unless @temperature.is_a?(Numeric) && @temperature >= 0.0 && @temperature <= 2.0
    ...
    def validate_top_p
      ...
      unless @top_p.is_a?(Numeric) && @top_p >= 0.0 && @top_p <= 1.0
    ...
    def validate_max_tokens
      ...
      unless @max_tokens.is_a?(Integer) && @max_tokens > 0
    ...
    def validate_truncation
      ...
      unless valid_truncation.include?(@truncation)
    ...
    def format_for_anthropic(params)
      ...
      if anthropic_params[:max_tokens]
        anthropic_params[:max_tokens_to_sample] = anthropic_params.delete(:max_tokens)
      end
    ...
    def format_for_cohere(params)
      ...
      if cohere_params[:max_tokens]
        cohere_params[:max_tokens] = cohere_params.delete(:max_tokens)
      end
      
      if cohere_params[:stop]
        cohere_params[:stop_sequences] = cohere_params.delete(:stop)
      end
    ...
    def format_for_gemini(params)
      ...
      if gemini_params[:max_tokens]
        gemini_params[:maxOutputTokens] = gemini_params.delete(:max_tokens)
      end
      
      if gemini_params[:temperature]
        gemini_params[:temperature] = gemini_params[:temperature]
      end
      
      if gemini_params[:top_p]
        gemini_params[:topP] = gemini_params.delete(:top_p)
      end
      
      if gemini_params[:top_k]
        gemini_params[:topK] = gemini_params.delete(:top_k)
      end
