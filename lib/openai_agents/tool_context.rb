...
module OpenAIAgents
  ...
  class ContextualTool < FunctionTool
    ...
    def initialize(function, context:, **options)
    ...
    def execute(**kwargs)
      ...
      begin
        ...
        if @function.respond_to?(:parameters)
          ...
          if params.any? { |type, name| name == :context }
  ...
  class Agent
    ...
    def add_contextual_tool(function, context: nil, **options)
      context ||= @context_manager&.get_context || ToolContext.new
      
      tool = ContextualTool.new(function, context: context, **options)
      add_tool(tool)
    end
    
    # Execute tool with context tracking
    alias_method :execute_tool_without_context, :execute_tool
