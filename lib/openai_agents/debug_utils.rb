  ...
  module DebugUtils
    ...
    class << self
      ...
      def log(level, message, context: {})
        ...
        when 'both'
          log_to_rails(level, message, context)
          log_to_file(level, formatted_message)
        else
          log_to_rails(level, message, context)
        end
      ...
      def tracing_enabled?
        !ENV.fetch('OPENAI_DISABLE_TRACING', 'false').downcase == 'true' && trace_debug_enabled?
      end

      def conversation_debug_enabled?
        ENV.fetch('OPENAI_AGENTS_DEBUG_CONVERSATION', 'false').downcase == 'true'
      end
      ...
      def log_to_rails(level, message, context)
        return unless defined?(Rails)
        
        formatted_message = context.any? ? "#{message} | Context: #{context.inspect}" : message
        Rails.logger.send(level, "[DEBUG] #{formatted_message}")
      end
