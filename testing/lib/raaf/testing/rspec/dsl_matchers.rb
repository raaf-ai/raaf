    ...
    module RSpec
      ...
      module DSLMatchers
        ...
        ::RSpec::Matchers.define :require_context_keys do |*expected_keys|
          ...
          failure_message do |agent_class|
            required_keys = if agent_class.respond_to?(:_required_context_keys)
        ...
        ::RSpec::Matchers.define :have_tools do |*expected_tools|
          ...
          failure_message do |agent_class|
            configured_tools = if agent_class.respond_to?(:_tools)
        ...
        ::RSpec::Matchers.define :have_successful_execution do
          match do |result|
            ...
            if result.respond_to?(:success?)
              ...
              [true, "true"].include?(result.success)
        ...
        ::RSpec::Matchers.define :have_output_structure do |expected_structure|
          match do |result|
            ...
            expected_structure.each do |key, expected_type|
              ...
              when :array
                @type_mismatches << "#{key}: expected Array, got #{actual_value.class}" unless actual_value.is_a?(Array)
              when :hash
                @type_mismatches << "#{key}: expected Hash, got #{actual_value.class}" unless actual_value.is_a?(Hash)
              when :string
