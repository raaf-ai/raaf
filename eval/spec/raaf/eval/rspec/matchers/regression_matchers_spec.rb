...
RSpec.describe "Regression Matchers" do
  ...
  let(:improved_output) do
    "Ruby is a dynamic, object-oriented programming language with elegant syntax and powerful metaprogramming capabilities."
  end
  ...
  describe "not_have_regressions matcher" do
    it "passes when no regressions detected" do
      expect(good_result).not_to have_regressions
    end

    it "fails when quality regresses" do
      expect do
        expect(regressed_result).not_to have_regressions
    ...
    it "supports severity filtering" do
      expect(good_result).not_to have_regressions.of_severity(:high)
    end
  ...
  describe "perform_better_than matcher" do
    ...
    context "when negated" do
      it "provides clear negated failure message" do
        expect(regressed_result).not_to perform_better_than(:baseline)
      rescue RSpec::Expectations::ExpectationNotMetError => e
  ...
  describe "have_acceptable_variance matcher" do
    ...
    it "checks token variance" do
      ...
      token_result.instance_variable_set(:@results, {
                                           run1: { success: true, output: "A",
                                                   usage: { input_tokens: 100, output_tokens: 50 } },
                                           run2: { success: true, output: "B",
                                                   usage: { input_tokens: 105, output_tokens: 48 } }
                                         })
    ...
    context "when negated" do
      it "provides clear negated failure message" do
        expect(consistent_results).not_to have_acceptable_variance
      rescue RSpec::Expectations::ExpectationNotMetError => e
  ...
  describe "edge cases" do
    it "handles missing usage data gracefully" do
      ...
      expect(eval_result).not_to have_regressions
    end
