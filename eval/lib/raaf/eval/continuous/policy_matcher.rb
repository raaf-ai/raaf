  ...
  module Eval
    module Continuous
      ...
      class PolicyMatcher
        ...
        # Returns empty array if backpressure is active to prevent queue overflow.
        # Returns empty array for evaluation-generated spans to prevent recursive evaluation.
        # Filters out manual-only policies (trigger_mode: "manual") since those
        # should only be triggered via the UI "Evaluate" button.
        #
        # @return [Array<EvaluationPolicy>] Policies to evaluate
        def policies_to_evaluate
          # Skip evaluation spans to prevent recursive evaluation loops
          # Evaluation-generated spans should not count toward policy sampling percentages
          if evaluation_span?
            RAAF.logger.debug "[ContinuousEval] Skipping span evaluation: span source is evaluation_run"
            return []
          end

          # Skip all evaluations when backpressure is active
          if Continuous.configuration.backpressure_active
            RAAF.logger.debug "[ContinuousEval] Skipping span evaluation: backpressure active"
            return []
          end

          # Filter to only automatic policies and apply sampling rules
          # Manual policies are shown in UI but don't auto-evaluate
          matching_policies.select do |policy|
            policy.automatic? && should_evaluate?(policy)
          end
        end
        ...
        def extract_attribute(attr)
          ...
          if span.respond_to?(attr)
            value = span.public_send(attr)
            return value if value.present?
          end

          # Try span_attributes hash (for SpanRecord with JSONB column)
          if span.respond_to?(:span_attributes) && span.span_attributes.is_a?(Hash)
          ...
          # Try span_data hash (for other span types)
        ...
        ##
        # Check if the span was generated by an evaluation run.
        # Evaluation-generated spans should not be re-evaluated to prevent
        # recursive loops and inflated sampling counts.
        # @return [Boolean] true if span source is "evaluation_run"
        def evaluation_span?
          source = extract_attribute(:source)
          source.to_s == "evaluation_run"
        end
